...
PR['registerLangHandler'](
    PR['createSimpleLexer'](
      ...
      , [ // :bind variable
        ...
        , [PR['PR_KEYWORD'], /^(?:%isopen|%found|%notfound|%rowcount|add|all|alter|and|any|apply|as|asc|authid|authorization|begin|between|body|break|bulk\s+collect|by|cascade|case|check|checkpoint|close|collate|column|commit|compress|compute|connect|constant|constraint|constructor|contains|context|continue|convert|create|cross|current|cursor|database|deallocate|declare|default|delete|deleting|deny|desc|deterministic|disk|distinct|distributed|drop|dummy|dump|editionable|else|elsif|end|errlvl|escape|exception_init|exceptions?|exec|execute\s+immediate|exists|exit|external|fetch|file|final|follows|for|foreign|from|full|function|goto|grant|group|having|hash|if|in|index|inner|insert|inserting|instantiable|intersect|into|is|isolation\s+level|join|key|kill|left|like[24c]?|limit|list|load|local|loop|map|match|matched|member|merge|nocompress|nocopy|nocycle|noneditionable|not|nulls\s+(first|last)|null|object|of|on|opaque|open|or\s+replace|or|ora_rowscn|order|out|outer|over|overriding|package|parallel_enable|partition|percent|pipe\s+row|pipelined|pivot|plan|pragma|precedes|primary|print|proc|procedure|public|raise|raise_application_error|range|raw|read|record|references|restrict|result|return(ing)?|revoke|right|rollback|rows?|save|schema|select|self|session_user|set|set\s+transaction|setuser|shutdown|some|start|static|storage|subpartition|subtype|table(space)?|then|to|top|trigger|truncate|type|unbounded|under|union|unique|unpivot|update|updating|use|user|using|values|varray|view|when|where|while|with|within|write)(?=[^\w-]|$)/i, null ]
          // All SQL and PL/SQL functions as of 12c rel 1, 2015
        , [PR['PR_FUNCTION'], /^(?:abs|acos|add_months|appendchildxml|approx_count_distinct|ascii|asciistr|asin|atan|atan2|avg|bfilename|bin_to_num|bitand|cardinality|cast|ceil|chartorowid|chr|cluster_details|cluster_distance|cluster_id|cluster_probability|cluster_set|coalesce|collect|compose|con_dbid_to_id|con_guid_to_id|con_name_to_id|con_uid_to_id|concat|convert|corr|cos|cosh|count|covar_pop|covar_samp|cume_dist|current_date|current_timestamp|dbtimezone|decode|decompose|deletexml|dense_rank|depth|determines|dimension|dump|empty_blob|empty_clob|existsnode|exp|extract|extractvalue|feature_details|feature_id|feature_set|feature_value|first|first_value|floor|following|from_tz|greatest|group_id|grouping|grouping_id|hextoraw|hierarchy|initcap|insertchildxml|insertchildxmlafter|insertchildxmlbefore|insertxmlafter|insertxmlbefore|instr[bc24]?|json_query|json_table|json_value|lag|last|last_day|last_value|lead|least|length[bc24]?|listagg|ln|lnnvl|localtimestamp|log|lower|lpad|ltrim|max|median|min|mod|model|months_between|nanvl|nchr|new_time|next_day|nls_charset_decl_len|nls_charset_id|nls_charset_name|nls_initcap|nls_lower|nls_upper|nlssort|nth_value|ntile|nullif|numtodsinterval|numtoyminterval|nvl|nvl2|ora_dst_affected|ora_dst_convert|ora_dst_error|ora_hash|ora_invoking_user|ora_invoking_userid|path|percent_rank|percentile_cont|percentile_disc|power|powermultiset|powermultiset_by_cardinality|preceding|prediction|prediction_bounds|prediction_cost|prediction_details|prediction_probability|prediction_set|range|rank|ratio_to_report|rawtohex|rawtonhex|regexp_count|regexp_instr|regexp_replace|regexp_substr|remainder|replace|reverse|round|row_number|rowidtochar|rowidtonchar|rpad|rtrim|rules|scn_to_timestamp|sessiontimezone|set|sign|sin|sinh|soundex|sqlcode|sqlerrm|sqrt|standard_hash|stats_binomial_test|stats_crosstab|stats_f_test|stats_ks_test|stats_mode|stats_mw_test|stats_one_way_anova|stats_wsr_test|stddev|stddev_pop|stddev_samp|substr[bc24]?|sum|sys_connect_by_path|sys_context|sys_dburigen|sys_extract_utc|sys_guid|sys_op_zone_id|sys_typeid|sys_xmlagg|sys_xmlgen|sysdate|systimestamp|tan|tanh|timestamp_to_scn|to_binary_double|to_binary_float|to_blob|to_char|to_clob|to_date|to_dsinterval|to_lob|to_multi_byte|to_nchar|to_nclob|to_number|to_single_byte|to_timestamp|to_timestamp_tz|to_yminterval|translate|treat|trim|trunc|tz_offset|uid|unistr|updatexml|upper|upsert|user|userenv|var_pop|var_samp|variance|vsize|width_bucket|xmlagg|xmlcast|xmlcdata|xmlcolattval|xmlcomment|xmlconcat|xmldiff|xmlelement|xmlexists|xmlforest|xmlisvalid|xmlparse|xmlpatch|xmlpi|xmlquery|xmlroot|xmlsequence|xmlserialize|xmltable|xmltransform)(?=[^\w-]|$)/i, null ]
          // SQL and PL/SQL data types, including "somecol%type", "sometable%rowtype".
        , [PR['PR_TYPE'], /^(?:[_a-z][\w\$#\.]*\s?%(row)?type|bfile|binary_double|binary_float|binary_integer|blob|boolean|char|child|clob|cursor|date|day|double\s+precision|dsinterval_unconstrained|float|identity|integer|interval|long|measures|month|natural|nchar|nclob|number|numeric|nvarchar2|pls_integer|ref ?cursor|second|sequential|simple_integer|time|time zone|timestamp|urowid|varchar2|varying array|xml|year)(?=[^\w-]|$)/i, null ]
          // Named exception
